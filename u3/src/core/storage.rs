...
#[cfg(test)]
pub mod test {
    use super::{DirectoryStorage, Storage, StorageReadError};
    use bytes::Bytes;
    ...
    fn corrupted_files() {
        ...
        let mut storage =
            DirectoryStorage::new(dir.path().to_owned(), 100, 10, 1024 * 1000).unwrap();
    ...
    fn extra_files_in_backup() {
        ...
        let mut storage =
            DirectoryStorage::new(dir.path().to_owned(), 100, 3, 1024 * 1000).unwrap();
