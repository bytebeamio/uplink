...
use tokio::select;
use crate::{PublishItem, DataRow, CONFIG};
...
impl SerializerStorageHandler {
    pub fn new(data_tx: Sender<DataRow>, buffers_batch_rx: Receiver<(String, Vec<PublishItem>)>, mqtt_client: AsyncClient) -> Self {
        let (topic_prefix, storages) = CONFIG.with(|c| {
            let topic_prefix = format!("/tenants/{}/devices/{}", c.auth.project_id, c.auth.device_id);
            let storages = c.cfg.streams
                .iter()
                .map(|(name, cfg)| {
                    (name.clone(), StorageState {
    ...
    pub async fn run(mut self) {
        ...
        let mut current_publish_task = self.current_publish.clone().map(|(_, publish)| mqtt_client.send_async(Request::Publish(publish)));
        loop {
            select! {
    ...
    fn write_buffer_to_storage(&mut self, (stream_name, data): (String, Vec<PublishItem>)) {
        let compress = self.storages.get(&stream_name)
    ...
    fn write_publish_to_storage(&mut self, stream_name: String, publish: Publish) {
        if !self.storages.contains_key(&stream_name) {
            ...
            self.storages.insert(stream_name.to_owned(), StorageState {
        ...
        if let Some(publish) = publish_to_write {
            match state.storage.write_packet(publish) {
                ...
                Err(StorageWriteError::InvalidPacket(e)) => {
                    log::error!("Found invalid packet when writing to storage for stream({stream_name}): {e:?}");
                }
            };
        }
    }
    ...
    fn get_next_publish(&mut self) -> Option<(String, Publish)> {
        if let Some((name, state)) = self.storages.iter_mut()
...
fn create_storage_for_stream(name: &str, config: &StreamConfig) -> StorageEnum {
    let (max_packet_size, directory) = CONFIG.with(|c| (c.cfg.mqtt.max_packet_size, c.cfg.persistence_path.join(name)));
    if config.persistence.max_file_count == 0 {
        StorageEnum::InMemory(storage::InMemoryStorage::new(name, config.persistence.max_file_size, max_packet_size))
    } else {
        match storage::DirectoryStorage::new(
            ...
            Err(e) => {
                log::error!("Failed to initialize disk backed storage for {name} : {e}, falling back to in memory persistence");
                StorageEnum::InMemory(storage::InMemoryStorage::new(name, config.persistence.max_file_size, max_packet_size))
            }
        }
    }
...
fn create_publish(stream_name: &str, data: &[PublishItem], topic_prefix: &str, compress: bool) -> Publish {
    let point_count = data.len();
    log::trace!("Data received on stream: {stream_name}; message count = {point_count}");

    let topic = format!("{topic_prefix}/events/{stream_name}/jsonarray{}", if compress { "/lz4" } else { "" });
