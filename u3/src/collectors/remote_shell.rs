...
async fn run_remote_shell(action: Action, data_tx: Sender<DataRow>) {
    send_action_response(&data_tx, &action.action_id, "ShellSpawned", 90, &[]).await;
    match run_remote_shell_impl(&action).await {
        Ok(_) => {
            send_action_response(&data_tx, &action.action_id, "Completed", 100, &[]).await;
        }
        Err(e) => {
            send_action_response(&data_tx, &action.action_id, "Failed", 100, &[e]).await;
        }
    }
}
