...
"#;
    ...
    pub fn initialize(auth_config: &str, uplink_config: &str) -> Result<Config, anyhow::Error> {
        ...
        replace_topic_placeholders(&mut config.downloader_metrics.topic, tenant_id, device_id);
        replace_topic_placeholders(&mut config.tunshell_metrics.topic, tenant_id, device_id);
