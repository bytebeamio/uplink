...
struct ActiveDownloadState {
    ...
    /// to be populated using above fields when starting download
    /// and loading state from disk
    #[serde(skip)]
    transient_download_state: Option<(File, Response)>,
}
...
impl DownloaderState {
    ...
    fn get_active_state(&mut self) -> Option<&mut ActiveDownloadState> {
        match &mut self {
            DownloaderState::Idle => None,
            DownloaderState::Downloading(state) => Some(state)
        }
    }
}
...
impl FileDownloader {
    ...
    pub fn new(
        ...
        let mut downloader_state = match std::fs::read_to_string(state_path.as_path()).context("couldn't read state file")
    ...
    async fn process_action(&mut self, action: Action) {
        if let DownloaderState::Downloading(curr_state) = &self.downloader_state {
            let action_id = curr_state.action.action_id.as_str();
            ...
            if let Err(e) = self.initiate_download(action).await {
                log::error!("Couldn't initiate download: {e}");
                self.bridge_tx.send_action_response(
                    ActionResponse::failure(&action.action_id, format!("Couldn't initiate download: {e}"))
                ).await;
            }
        }
    }
    ...
    async fn increment_download(&mut self) {
        let state = match &mut self.downloader_state {
            DownloaderState::Idle => unreachable!(),
            DownloaderState::Downloading(state) => state,
        };

        /// In one iteration we fetch 1 percent of file, with min as 1KB and max as 1MB
        let download_size = lerp(1024, state.content_length / 100, 1024 * 1024);

        /// If server is not responding, it will take upto 10s for the action to get cancelled
        let fetch_size = self.client.get(state.url.as_str())
            .timeout(Duration::from_secs(10))
            .header("Range", format!("{}-{}", state.bytes_written, min(state.bytes_written + download_size, state.content_length)))
            .send().await;
    ...
    pub async fn start(mut self) {
        ...
        /// If there is no active download, we wait for an action
        /// If there is an active download, we do the download operation and every 100ms check for an
        /// incoming action (which might be cancellation)
        loop {
    ...
    async fn initiate_download(&mut self, action: Action) -> Result<(), String> {
        ...
        let download_path = self.config.path.join(action.name.as_str()).join(payload.file_name.as_str());
        ...
        self.downloader_state = DownloaderState::Downloading(ActiveDownloadState {
            ...
            transient_download_state: None,
        });
        Ok(())
    }

    fn open_file_and_initiate_request(&mut self) -> (File, )
}
...
impl DownloadFile {
    fn verify_checksum(&self) -> bool {
        // let Some(checksum) = &self.checksum else { return true; };
