...
impl LogFileReader {
    ...
    pub async fn start(self) {
        loop {
            if let Err(e) = self.run_once().await {
                log::error!("log reader::({}) stopped with an error: {e}", self.name);
            } else {
                log::info!("log reader::({}) stopped, restarting", self.name);
            }
    ...
    async fn run_once(&self) -> Result<(), Error> {
        let mut cmd =
            Command::new(self.config.cmd.get(0).unwrap()).args(&self.config.cmd[1..])
                .stdin(Stdio::piped())
