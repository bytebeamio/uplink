...
pub enum Error {
    ...
    Cancelled(String),
    #[error("Action stalled by uplink restart")]
    Restart,
}
...
impl ActionsBridge {
    pub fn new(
        ...
        Self {
            ...
            action_redirections,
            current_action: ActionState { main_action: None, parallel_actions: HashSet::new() },
            shutdown_handle,
            ctrl_rx,
            ctrl_tx,
        }
    ...
    fn clear_current_action(&mut self) {
        self.current_action.main_action.take();
    }
    ...
    pub async fn start(&mut self) -> Result<(), Error> {
        ...
        // Mark parallel actions as failed
        let parallel_actions: Vec<String> = self.current_action.parallel_actions.drain().collect();
        ...
        loop {
            select! {
                ...
                _ = &mut self.current_action.main_action.as_mut().map(|a| &mut a.timeout).unwrap_or(&mut end) => {
                    let curret_action = self.current_action.main_action.as_mut().unwrap();
    ...
    async fn handle_action(&mut self, action: Action) {
        ...
        if let Some(current_action) = &self.current_action.main_action {
    ...
    async fn handle_cancellation(&mut self, action: Action) -> Result<(), Error> {
        let action_id = action.action_id.clone();
        let Some(current_action) = self.current_action.main_action.as_ref() else {
        ...
        self.current_action.main_action.as_mut().unwrap().cancelled_by = Some(action_id.clone());
    ...
    fn save_current_action(&mut self) -> Result<(), Error> {
        ...
        self.current_action.write_to_disk(path)?;
    ...
    fn load_saved_action(&mut self) -> Result<(), Error> {
        ...
        if path.is_file() {
            self.current_action = ActionState::read_from_disk(path)?;
            if let Some(current_action) = &self.current_action.main_action {
                info!(
    ...
    fn try_route_action(&mut self, action: Action) -> Result<(), Error> {
        ...
        if action.name == TUNSHELL_ACTION {
            self.current_action.parallel_actions.insert(action.action_id);
            return Ok(());
        }

        self.current_action.main_action = Some(CurrentAction::new(action, deadline));
    ...
    async fn forward_action_response(&mut self, mut response: ActionResponse) {
        ...
        if self.current_action.parallel_actions.contains(&response.action_id) {
            self.forward_parallel_action_response(response).await;

            return;
        }

        let inflight_action = match &mut self.current_action.main_action {
            Some(v) => v,
            None => {
        ...
        if response.is_done() {
            let mut action = self.current_action.main_action.take().unwrap().action;
    ...
    async fn forward_parallel_action_response(&mut self, response: ActionResponse) {
        info!("Action response = {:?}", response);
        if response.is_completed() || response.is_failed() {
            self.current_action.parallel_actions.remove(&response.action_id);
        }
...
struct SaveActions {
    ...
    #[serde(default)]
    pub parallel_actions: HashSet<String>,
}
...
struct ActionState {
    main_action: Option<CurrentAction>,
    parallel_actions: HashSet<String>,
}

impl ActionState {
    pub fn write_to_disk(&mut self, path: PathBuf) -> Result<(), Error> {
        let (action, timeout) = match self.main_action.take() {
            Some(a) => (Some(a.action), Some(a.timeout.deadline() - Instant::now())),
            _ => (None, None),
        };
        let save_action = SaveActions {
            action,
            timeout,
            parallel_actions: mem::take(&mut self.parallel_actions),
        };
    ...
    pub fn read_from_disk(path: PathBuf) -> Result<Self, Error> {
        ...
        let (main_action, parallel_actions) = match serde_json::from_slice(&read)? {
            SaveActions { action: Some(action), timeout: Some(timeout), parallel_actions } => (
                Some(CurrentAction {
                    action,
            ...
            SaveActions { parallel_actions, .. } => (None, parallel_actions),
        };
        fs::remove_file(path)?;

        Ok(ActionState { main_action, parallel_actions })
    }
}
