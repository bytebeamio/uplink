...
impl<C: MqttClient> Serializer<C> {
    ...
    fn select_storage<'a>(&mut self, topic: &str) -> Result<&mut Storage, Error> {
        let storage = self
            .storage_map
    ...
    async fn catchup(&mut self) -> Result<Status, Error> {
        ...
        loop {
            let storage = match self.storage_map.iter_mut().next() {
                Some((_, s)) => s,
